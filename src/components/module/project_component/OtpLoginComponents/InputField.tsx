...
const validate = () => {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setSubmitting(true);
  {showNum?  globalOtp(setApiResponse, {mobile:formData.mobile,country_code:formData.country_code},setSubmitting): globalOtp(setApiResponse, {email:formData.email},setSubmitting)}
  ...
  useEffect(() => {
    ...
    if (apiResponse) {
      setSubmitting(false);
      setShowOtp(true);
     showNum? setApiRequestData({mobile:formData.mobile}):setApiRequestData({email:formData.email})
    }
  }, [apiResponse]);
...
setErrors((prev)=>({ ...prev, mobile: "",email:""}))
  ...
  return (
   ...
   if (!allowedEmailChars.test(e.key) && !allowedControlKeys.includes(e.key)) {
     e.preventDefault(); // Block any character not allowed in an email
      ...
      <button
        type="submit"
        className="w-full  mt-4 m-0  theme-button  rounded-md  transition"
      >
