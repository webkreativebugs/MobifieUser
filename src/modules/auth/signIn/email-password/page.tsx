...
const validateField = (name: string, value: string) => {
  ...
  const validate = () => {
    ...
    setTouched((prev) => {
      ...
      Object.keys(errors).forEach((k) => { allTouched[k as keyof typeof allTouched] = true; });
      return allTouched;
    });
    return valid;
  };
 ...
 <div className="flex justify-center items-center min-h-screen app-container">
    ...
    <div className="flex flex-col  items-center justify-center  md:w-1/2  relative  ">
     <img src={logo} className='absolute top-0 left-0 w-[80px]' />
      <div className="w-full max-w-xs ">
        ...
        <h1 className="text-center text-3xl font-semibold mb-2">Welcome Back</h1>
        ...
        <form className="space-y-5" onSubmit={handleSubmit} >
          <div className='error-wrapper'>
            ...
            <div className="flex items-center border-gray-300 border-2 rounded mt-1 bg-white">
              ...
              />
               ...
               {touched.email && errors.email && <div className="text-danger small mt-0 error-tooltip ">   {errors.email}</div>}
  ...
  <div className="error-wrapper">
      ...
      {touched.password && errors.password && (
        <div className="text-danger text-sm mt-1 error-tooltip">{errors.password}</div>
      )}
    </div>}
          
          <div className="flex justify-between items-center">
           {showOtp?    <div className="small text-muted mb-1">
            {disable ? (
              <div className="d-flex gap-0  w-auto primary-text-color" ><span>Resend OTP in:</span> <span className="fw-bold w-2 ">{timeLeft}s</span></div>
            ) : (
              <a

                className=" btn-hover-anim mb-xs-10 primary-text-color "
                // onClick={ResendOtp}
              ...
              >
                Resend OTP
                      ...
                      :<a  href="/forgetPassword" className="text-sm primary-text-color hover:underline">Forgot password?</a>
           }
          </div>
          
          <button
            type="submit"
            className="w-full   p-2 theme-button  rounded  transition"
          >
          {!submitting?(showOtp?"Send OTP" : "Login"):"Submitting..."}
          </button>
            <div className="flex justify-center items-center">
           {!showOtp?<a className="flex items-center text-sm primary-text-color cursor-pointer" onClick={()=>{
              setShowOtp(true);
              setErrors(prev => ({...prev, email: '',password:''}));
               setTimeLeft(30)
            }} >Login with OTP  </a>
                    :<a className="flex items-center text-sm primary-text-color cursor-pointer" onClick={()=>{
              setShowOtp(false);
              setErrors(prev => ({...prev, email: '',password:''}));
              setTimeLeft(30)
 ...
 <div className="relative hidden md:w-1/2 card md:flex justify-center items-center p-6 border-2 overflow-hidden rounded-lg">
  {/* Background Shapes */}
